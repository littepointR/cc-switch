...
impl McpApps {
    ...
    pub fn is_enabled_for(&self, app: &AppType) -> bool {
        match app {
            ...
            AppType::Pi => false, // Pi doesn't support MCP by default
    ...
    pub fn set_enabled_for(&mut self, app: &AppType, enabled: bool) {
        match app {
            ...
            AppType::Pi => {}      // Pi doesn't support MCP by default, ignore
...
pub struct McpRoot {
    ...
    /// Pi MCP 配置（当前默认不支持 MCP，保留字段用于配置兼容）
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
    pub pi: McpConfig,
...
impl AppType {
    ...
    /// - Additive mode (true): All providers are written to live config (OpenCode, Pi, OpenClaw, Hermes)
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, claude-desktop, codex, gemini, opencode, pi, openclaw, hermes。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, claude-desktop, codex, gemini, opencode, pi, openclaw, hermes."),
            )),
        }
    }
}
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::Pi => continue,       // Pi doesn't support MCP by default
